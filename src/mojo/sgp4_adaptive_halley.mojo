...
fn fast_sin_cos_adaptive(x: Vec) -> Tuple[Vec, Vec]:
    """Width-generic fast math using parametric function"""
...
fn sgp4_propagate_adaptive(c: SGP4Constants, tsince: Float64) -> Tuple[Vec, Vec, Vec, Vec, Vec, Vec]:
    ...
    # Halley's method - 3rd order convergence (2 iterations instead of 3)
    # f(E) = E - e*sin(E) - M
    # f'(E) = 1 - e*cos(E)
    # f''(E) = e*sin(E)
    # E_new = E - 2*f*f' / (2*f'^2 - f*f'')
    ...
    var sine = sc[0]
    var cose = sc[1]
    var f = e - c.ecco * sine - xmp_drag
    var fp = 1.0 - c.ecco * cose
    var fpp = c.ecco * sine
