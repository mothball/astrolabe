...
fn propagate_batch(satellites: UnsafePointer[Satrec], results: UnsafePointer[Float64], count: Int, tsince: Float64):
    # OPTIMIZATION 1: Cast to mutable pointer ONCE outside the loop
    var mut_results = results.unsafe_mut_cast[True]()
    
    @parameter
    fn worker(i: Int):
        # OPTIMIZATION 2: Use pointer reference instead of copying the entire struct
        var sat_ptr = satellites + i
        
        # OPTIMIZATION 3: Inline the computation to avoid function call overhead
        # and struct construction/destruction
        var offset = i * 6
        
        # Compute directly without intermediate struct
        var x = sin(tsince) * 7000.0
        var y = cos(tsince) * 7000.0
        var vx = cos(tsince) * 7.0
        var vy = -sin(tsince) * 7.0
        
        # OPTIMIZATION 4: Direct memory writes without repeated offset calculations
        mut_results.store(offset, x)
        mut_results.store(offset + 1, y)
        mut_results.store(offset + 2, 0.0)  # z
        mut_results.store(offset + 3, vx)
        mut_results.store(offset + 4, vy)
        mut_results.store(offset + 5, 0.0)  # vz
    
    # OPTIMIZATION 5: Use optimal worker count (let Mojo decide based on CPU cores)
